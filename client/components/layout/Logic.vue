<template>
  <div class="logic-table">
    <div>
      ...
      <!-- 互斥表格 -->
      <div class="table-content" v-show="imshowArr.length > 1 && imshowArr[0].length > 1">
        <caption class="table-caption" v-html="'\u2714：纵轴执行时关闭横轴（左键）<br/>\u2716：纵轴执行后隐藏横轴（右键）'" />
        <table class="table-main">
          <tbody>
            <tr v-for="(colItem, colIndex) in imshowArr" :key="colIndex">
              <td
...
import SelectLabel from '@/components/popups/SelectLabel.vue';
...
export default {
  ...
  methods: {
    ...
    panelShow(index = false) {
      ...
      this.$popup.show({
        ...
        componentConfig: {
          ...
          default: SelectLabel,
          props: {
            selectAxis: ['col', 'row'][index],
            selectLabel: this.selectLabel,
            labelList: this.labelList,
            outputHandler: ({ labels, selectAxis }) => {
              const selectLabel = deepCopy(this.selectLabel);
              selectLabel[selectAxis] = deepCopy(labels);
              this.setPulicModule({ selectLabel });
            }
          }
        }
    ...
    setHoverClass(rowIndex, colIndex) {
      ...
      if (
        (rowIndex === this.hoverItem[0] && colIndex === 0) ||
