    ...
    <gree-page class="page-music-paly no-navbar" style="background: #bbb" @touchstart.native="listShow = false">
      ...
      <div class="page-main">
        ...
        <transition-group tag="div" name="list">
          <div class="list-title" v-show="listShow" v-text="`播放列表（共${listLen}首）`" key="title" @touchstart.stop />
          <div class="song-list" v-show="listShow" key="list" @touchstart.stop>
            <!-- 列表内容 -->
            <div
        ...
        <div class="lyric">
          <!-- 歌词内容 -->
          <div class="lyric-content" @touchstart="setDragLyric(true)" @touchend="setDragLyric(false)" @touchmove="dragLyric" @scroll="dragLyric">
            <!-- 内框 -->
            <div class="lyric-box">
              <p
                class="lyric-text"
...
export default {
    ...
    btnList() {
      const result = [
        ...
        },
        {
          ...
          size: 'big',
          action: () => {
            this.setMusicData({ musicPlaying: !this.musicPlaying });
          }
        },
        {
  ...
  methods: {
    ...
    updateLyric() {
      ...
      // 定位歌词
      this.lyricIndex = this.getLyricPos(
        currentTime,
        this.lyricArr.map(item => item.time)
      );
    },
    ...
    dragLyric() {
      ...
      this.dragLyricIndex = this.getLyricPos(
        scrollTop,
        this.lyricTop.map(item => item.scrollTop)
      ); // 根据滚动条高度选择拖拽中心的歌词
    },
    // 返回歌词的位置,支持时间/拖动距离检索
    getLyricPos(val, arr) {
