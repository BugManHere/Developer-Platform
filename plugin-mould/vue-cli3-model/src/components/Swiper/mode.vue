...
export default {
  ...
  data() {
    return {
      ...
      rightLen: 3,
      imgList: [
        {img: require('@/assets/img/mode/auto.png'), name: this.$language('mode.auto'), index: 0},
        {img: require('@/assets/img/mode/cool.png'), name: this.$language('mode.cool'), index: 1},
        {img: require('@/assets/img/mode/dry.png'), name: this.$language('mode.dry'), index: 2},
        {img: require('@/assets/img/mode/fan.png'), name: this.$language('mode.fan'), index: 3},
        {img: require('@/assets/img/mode/heat.png'), name: this.$language('mode.heat'), index: 4},
      ],
      modeName: 'auto'
    };
  },
  ...
  methods: {
    ...
    setFanName(index) {
      if (index === undefined) {
        this.modeName = this.imgList[this.swiperIndex].name;
        return;
      }
      const toIndex = this.countIndex(this.swiperIndex, index - this.leftLen);
      this.modeName = this.imgList[toIndex].name;
    }
  }
};
</script>
