...
const homeConfig = {
  ...
  computed: {
    functionList() {
      ...
      !this.$store.state.dataObject.functype && this.g_moreOption && this.g_moreOption.voiceSkill && result.push({
        url: require('../../assets/img/voiceSkill.png'),
        name: '语音技能',
        key: 'Voice'
      });
      // 如果1.
      !this.$store.state.dataObject.functype && this.$store.state.devOptions.statueJson2.includes('ElcEn') && this.$store.state.dataObject.ElcEn && result.push({
